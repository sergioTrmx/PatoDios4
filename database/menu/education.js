const education = (prefix, botName, ownerName) => {
	return `
┏ *〈 ${botName} 〉*
╿
┯ *〈 𝑰𝑵𝑭𝑶𝑹𝑴𝑨𝑪𝑰𝑶𝑵 𝑫𝑬𝑳 𝑩𝑶𝑻 〉*
   ╽
   ┠≽ *𝑃𝑟𝑒𝑓𝑖𝑗𝑜* : 「  ${prefix}  」
   ┠≽ *𝐶𝑟𝑒𝑎𝑑𝑜𝑟* : ${ownerName}
   ┠≽ *𝑉𝑒𝑟𝑠𝑖𝑜𝑛* : 0.0.5
   ╿
┯┷ *〈 𝐼𝑁𝐹𝑂𝑅𝑀𝐴𝐶𝐼𝑂𝑁 〉*
╽
┠≽ *${prefix}info* (error)
┃ *Desc* : 𝑀𝑜𝑠𝑡𝑟𝑎𝑟 𝑑𝑒𝑡𝑎𝑙𝑙𝑒𝑠 𝑑𝑒𝑙 𝑏𝑜𝑡
┠──────────────╼
┠≽ *${prefix}blocklist*
┃ *Desc* : 𝑀𝑜𝑠𝑡𝑟𝑎𝑟 𝑢𝑠𝑢𝑎𝑟𝑖𝑜𝑠 𝑏𝑙𝑜𝑞𝑢𝑒𝑎𝑑𝑜𝑠
┠──────────────╼
┠≽ *${prefix}chatlist* (error)
┃ *Desc* : 𝑀𝑜𝑠𝑡𝑟𝑎𝑟 𝑡𝑜𝑑𝑜𝑠 𝑙𝑜𝑠 𝑢𝑠𝑢𝑎𝑟𝑖𝑜𝑠 𝑑𝑒𝑙 𝑐ℎ𝑎𝑡
┠──────────────╼
┠≽ *${prefix}ping*
┃ *Desc* : 𝑀𝑜𝑠𝑡𝑟𝑎𝑟 𝑣𝑒𝑙𝑜𝑐𝑖𝑑𝑎𝑑 𝑑𝑒𝑙 𝑏𝑜𝑡
┠──────────────╼
┠≽ *${prefix}bugreport* <text>
┃ *Desc* : 𝑅𝑒𝑝𝑜𝑟𝑡𝑎𝑟 𝑏𝑢𝑔 𝑜 𝑒𝑟𝑟𝑜𝑟
╿
┷┯ *〈 EDUCATION 〉*
   ╽
   ┠≽ *${prefix}wiki* <query>
   ┃ *Desc* : 𝐵𝑢𝑠𝑐𝑎𝑟 𝑎𝑙𝑔𝑜 𝑒𝑛 𝑤𝑖𝑘𝑖𝑝𝑒𝑑𝑖𝑎 [indo]
   ┠──────────────╼
   ┠≽ *${prefix}wikien* <query>
   ┃ *Desc* : 𝐵𝑢𝑠𝑐𝑎𝑟 𝑠𝑒𝑔𝑢𝑛 𝑤𝑖𝑘𝑖𝑝𝑒𝑑𝑖𝑎 [english]
   ┠──────────────╼
   ┠≽ *${prefix}nulis* <text>
   ┃ *Desc* : 𝑇𝑒𝑥𝑡𝑜 𝑒𝑛 ℎ𝑜𝑗𝑎
   ┠──────────────╼
   ┠≽ *${prefix}quotes*
   ┃ *Desc* : 𝐸𝑛𝑣𝑖𝑎𝑟 𝑟𝑎𝑛𝑑𝑜𝑚 𝑐𝑖𝑡𝑎𝑠
   ┠──────────────╼
   ┠≽ *${prefix}quotes2*
   ┃ *Desc* : 𝐸𝑛𝑣𝑖𝑎𝑟 𝑟𝑎𝑛𝑑𝑜𝑚 𝑐𝑖𝑡𝑎𝑠 2
   ┠──────────────╼
   ┠≽ *${prefix}tafsirmimpi* <dream>
   ┃ *Desc* : 𝐼𝑛𝑡𝑒𝑟𝑝𝑟𝑒𝑡𝑎𝑟 𝑠𝑢𝑒𝑛̃𝑜
   ┠──────────────╼
   ┠≽ *${prefix}translate* <language_code>|<text>
   ┃ *Desc* : 𝑇𝑟𝑎𝑑𝑢𝑐𝑐𝑡𝑜𝑟
   ┠──────────────╼
   ┠≽ *${prefix}artinama* <name>
   ┃ *Desc* : 𝐼𝑛𝑡𝑒𝑟𝑝𝑟𝑒𝑡𝑎𝑟 𝑛𝑜𝑚𝑏𝑟𝑒𝑠
   ╿ *${ownerName}*,
   ╰╼≽ *𝐶𝑟𝑒𝑎𝑑𝑜𝑟 © ${botName}`
}
exports.education = education
